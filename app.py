...
from plotly.subplots import make_subplots  # Bug修复：导入make_subplots
...
def render_c_end_home():
    ...
    with st.expander("🛡️ 查阅《东林康养：医疗级卫生与代际互动安全规范》", expanded=False):
        st.markdown("""
        **1. 绝对物理隔离**：长者颐养区与幼儿早教区配备**独立新风系统**，日常互不交叉。代际互动仅在每日2次紫外线消毒的“第三空间”进行。
        **2. 医疗级准入筛查**：所有参与“老幼同乐”的长者，均已通过系统对接私立医院的**严格传染病筛查（持绿码准入）**，系统自动拦截发热或流感高风险人员。
        **3. 任务导向非强制**：拒绝“干瞪眼”式强迫社交。算法精准匹配长者特长，若幼儿在互动中表现出抵触情绪，专业护工将100%无条件立即终止互动。
        **4. 兜底保障机制**：预约费用已自动包含**双向交叉感染险**与意外险。
        """)
    
    # 强制健康勾选（堵住传染病漏洞）
    health_promise = st.checkbox("☑️ 我已阅读上述隔离规范，并承诺本次预约的家庭成员近期无发热、咳嗽及其他呼吸道/传染性疾病。")
   
    st.divider()
   
    if st.button("生成预约方案与动态报价", use_container_width=True, type="primary"):
        if not health_promise:
            st.error("⚠️ 请先阅读并勾选《健康申报与安全承诺》以保障园区一老一小的共同安全。")
            return # 阻断提交
